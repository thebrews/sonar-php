...
public abstract class PhpRuleRepository extends RuleRepository {
  ...
  protected PhpRuleRepository(String key, String language, Settings settings) {
    super(key, language);
    Logger LOG = LoggerFactory.getLogger(PhpRuleRepository.class);
  ...
  Logger LOG = LoggerFactory.getLogger(PhpRuleRepository.class);
    ...
    for (File userExtensionXml : getFileSystem().getExtensions("rules", "xml")) {
      rules.addAll(getParser().parse(userExtensionXml));
    }

    LOG.info("number of rules : " + String.valueOf(rules.size()));
